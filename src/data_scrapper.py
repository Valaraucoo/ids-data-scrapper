...
def fetch_and_save_data(line_no=LINE_NO, loops_delay=DELAY_BETWEEN_LOOPS, stops_delay=DELAY_BETWEEN_STOPS) -> None:
    ...
    while True:
        ...
        for current_stop in STOPS_LINE:
            ...
            if firebase_push_data(TO_SAVE, weather, flow_data_for_stop[current_stop], FIREBASE_BASE_DOC):
                if len(TO_SAVE) > 0:
                    report_info(f"{len(TO_SAVE)} records to {HEADER}Firebase/{FIREBASE_BASE_DOC}{ENDC} at stop no. {current_stop}.")
                else:
